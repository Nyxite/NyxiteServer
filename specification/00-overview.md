...
6. **Shared C# with desktop; contract to others.** Domain models, DTOs, crypto, and CRDT glue are shared C# with the Avalonia desktop client. Web and Android consume the emitted OpenAPI contract (and the same wire protocols), not a shared library.
...
| **Handwritten ink** | Vector strokes | LWW / version-vector | Encrypted blob (vector stroke format) |

The text/ink split (CRDT for text, LWW/version-vector for ink and binary) is decision **[OD-4]**.
...
- **Sync** — cross-device engine; per-file policies `server-default`, `pinned-local`, `excluded` **[OD-3]**.
...
|-------|---------------------|
| **0 Foundations** | Keycloak + 2FA; **device-key enrollment + key directory + recovery escrow**; Postgres + blob store (ciphertext); structure/metadata CRUD with encrypted names; OpenAPI. |
...
| **3 Handwriting** | Ink vector strokes as encrypted blobs; LWW / version-vector sync. |
...
- **Recovery key** — user-held secret that unlocks a self-recovery escrow; the **only** recovery path (no server escrow).
