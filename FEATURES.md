...
ASP.NET Core (C#) backend. The core service: API, sync engine, real-time collaboration **relay**, end-to-end encryption support, authentication, and the admin API (the operator dashboard is the separate [Admin](https://github.com/Nyxite/Nyxite/blob/main/features/admin.md) component).
...
- **Native, server-owned auth** — password (**Argon2id over a secret HMAC-SHA256 pepper**, rotatable, held outside the DB) + required TOTP, plus co-equal **passkeys (WebAuthn)**; the server **issues its own access + refresh tokens**. The login password never feeds content-key derivation; decryption is governed by device/identity keys. **Keycloak/OIDC is a pluggable enterprise IdP** resolving to the same internal token — a **licensed enterprise feature** (gated off in community mode; see *Licensing & enterprise entitlement* below). (See [SPECIFICATION §10](https://github.com/Nyxite/Nyxite/blob/main/docs/SPECIFICATION.md).)

## Admin API & audit

- Exposes the **admin API** (`/admin/**`) and owns the **audit log** plus signed-export generation; the operator UI is **not** built into the server — it is the separate **[Admin dashboard](https://github.com/Nyxite/Nyxite/blob/main/features/admin.md)** (`NyxiteAdmin`), which consumes this API
...
- Owns the **client-side entitlement layer** for self-hosting licensing (L-1–L-7). The **vendor-side** license server is the separate [`NyxiteLicense`](https://github.com/Nyxite/NyxiteLicense) component; this server is its *client*.
- **Offline token verification** — embeds the license **hybrid Ed25519 + ML-DSA-65** public key(s) and verifies the per-instance token locally (both signature halves) at startup + on a timer. **Boot never blocks**; absent/invalid token ⇒ **community mode** (free non-commercial: unlimited seats, group cap 16, all core features). Two embedded keys allow signing-key rotation.
...
- **Feature-flag gates** — resolves the entitlement set into flags checked at each enterprise-feature site: **SSO/OIDC**, **enterprise reader-groups**, **advanced admin overrides** (quota/group-size override, scoped/custom RBAC), **signed audit-log export**, **group size > 16** (community pins `group_max_members = 16` + disables the override), and the **self-hosted helpdesk desk** (SUP-11 — an opted-in operator's own `NyxiteSupport` desk refuses to function without a valid lease; the central helpdesk stays free).
...
## Support relay (bug reporting)

- The server is an **authenticating relay** for in-app bug reports to the central, vendor-side **[Nyxite Support](https://github.com/Nyxite/Nyxite/blob/main/features/support.md)** helpdesk (`NyxiteSupport`) — it **stores no ticket**. It authenticates the submitter (account or guest-share session), tags the report with the **instance fingerprint** (anti-spam) + an **opaque user reference**, forwards it, and proxies a reporter's own "My tickets" reads/replies by that opaque reference (SUP-7).
- Support endpoints (`POST /support/reports`, `GET /support/tickets[/{id}]`, `POST /support/tickets/{id}/replies`) are exposed when reporting is enabled for the instance (a `support.enabled` capability flag, **on by default**). **Every** instance relays to the maintainer's **central** desk by default (SUP-9); an **enterprise operator who opts into their own desk** (SUP-10–SUP-13) points the relay at their **local** desk instead, and only confirmed bugs route upstream. No operator/management endpoints live on the server — those are on `NyxiteSupport`.
- Runs on the **consensual, non-E2EE support plane** (disjoint from content — no content key or content-plane ciphertext); the report body + redacted screenshot are authored and redacted **client-side** before submission. See [Nyxite Support](https://github.com/Nyxite/Nyxite/blob/main/features/support.md), server `specification/04 §4.9` and `14 §14.9`, and [OPEN-DECISIONS.md](https://github.com/Nyxite/Nyxite/blob/main/docs/OPEN-DECISIONS.md) (SUP-1–SUP-13).

## Open questions

See [../docs/OPEN-DECISIONS.md](https://github.com/Nyxite/Nyxite/blob/main/docs/OPEN-DECISIONS.md). The server-owned items are now **resolved** there: native auth (server-owned password+TOTP / passkeys, server-issued tokens, Keycloak as a pluggable enterprise IdP — #9), key recovery (client-encrypted recovery blob, no server/admin escrow), metadata boundary (encrypted names; structure-hiding deferred to Phase 6), multi-device enrollment, fragment-key sharing, rotation-based revocation, key-directory trust (TLS + hybrid Ed25519 + ML-DSA-65 self-signature for v1, with the key-transparency **log** required at Phase 4.3 for groups per G-3; safety-number verification stays optional Phase 6), sync-policy semantics (`{ server-default, excluded }`), background auto-sync relay (#11), and the CRDT/LWW split.
