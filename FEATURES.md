...
- The server is an **authenticating relay** for in-app bug reports to the central, vendor-side **[Nyxite Support](support.md)** helpdesk (`NyxiteSupport`) — it **stores no ticket**. It authenticates the submitter (account or guest-share session), tags the report with the **instance fingerprint** (anti-spam) + an **opaque user reference**, forwards it, and proxies a reporter's own "My tickets" reads/replies by that opaque reference (SUP-7).
- Support endpoints (`POST /support/reports`, `GET /support/tickets[/{id}]`, `POST /support/tickets/{id}/replies`) are exposed **only when reporting is enabled** for the instance (a `support.enabled` capability flag; v1 = the maintainer's official instance(s) — SUP-9). No operator/management endpoints live on the server — those are on `NyxiteSupport`.
- Runs on the **consensual, non-E2EE support plane** (disjoint from content — no content key or content-plane ciphertext); the report body + redacted screenshot are authored and redacted **client-side** before submission. See [Nyxite Support](support.md), server `specification/04 §4.9` and `14 §14.9`, and [OPEN-DECISIONS.md](../docs/OPEN-DECISIONS.md) (SUP-1–SUP-9).
